...
def get_expbal_from_session(intent, session):
    ...
    if session.get('attributes', {}) and "expBal" in session.get('attributes', {}):
        ...
        speech_output = "Your express balance is " + exp_bal + \
                        ". Goodbye."
        should_end_session = True
    else:
...
def get_swipes_from_session(intent, session):
    ...
    if session.get('attributes', {}) and "swipesBal" in session.get('attributes', {}):
        ...
        speech_output = "Your flex balance is " + swipes_bal + \
                        ". Goodbye."
        should_end_session = True
    else:
...
def get_laundry_from_session(intent, session):
    ...
    if session.get('attributes', {}) and "laundryInfo" in session.get('attributes', {}):
        laundry_info = session['attributes']['laundryInfo']
        speech_output = "Your flex balance is " + laundry_info + \
                        ". Goodbye."
        should_end_session = True
    else:
        speech_output = "I'm not sure what your laundry is. "
        should_end_session = False
...
def on_intent(intent_request, session):
    ...
    if intent_name == "WhatsMySwipesIntent":
